...
- [x] Boss fight + ability unlock: Death (2-phase), Air Dash reward
  - DeathBoss.cs — sealed EnemyBase subclass: Phase 1 ground (ScytheSweep rush+OverlapBox, TeleportSlash snap-behind+swing), Phase 2 aerial at ≤30% HP (AerialDive snap-above→dive+AOE, ScytheBarrage 3-rapid-swings); BossStartedEvent/BossDefeatedEvent; GrantAirDash() sets PlayerMovement.CanAirDash=true + ItemPickedUpEvent; SetFacing() modifies localScale.x sign; static Collider[8] hitBuffer no-alloc
  - BossTrigger (in DeathBoss.cs) — one-shot OnTriggerEnter, fires callback, self-destructs
  - VerticalSliceBootstrap.cs — BuildBossTrigger() (BoxCollider at col 37, wires BossTrigger.OnPlayerEntered → SpawnDeathBoss); SpawnDeathBoss() builds EnemyDataSO (HP=300, ATK=25, DEF=5, EXP=500) + Capsule visual + DeathBoss component; emits BossStartedEvent
  - web-prototype/public/vertical-slice-demo.html — full Death boss fight: spawnBoss()/updateBoss()/updateBossAttack()/killBoss()/drawBoss()/renderBossBody(); 4 attack patterns (sweep/teleport/dive/barrage); boss HP bar top-center; screenFlash() state; Air Dash [X] mid-air after defeat; airDashUnlocked/airDashes/airDashCd state; reset() tears down boss state
- [x] Phase 8: Ability progression + gating — Double Jump, three gate types, dramatic fanfare
  - AbilityGate.cs — three GateType variants (Spectral shimmer/IronGate portcullis/CrackedWall rubble); solid BlockerCollider + child Sensor trigger pattern; checks PlayerMovement.CanDoubleJump/CanAirDash/CanWallJump; 0.5s dissolve coroutine on unlock; ScreenFlashEvent + CameraShakeEvent
  - AbilityAcquisitionPresenter.cs — static Show() factory; UGUI overlay SortingOrder=1000; Time.timeScale=0; Phase 1: 0.6s purple flash pulse + "POWER ACQUIRED" header; Phase 2: black cinematic bars slide in (0.35s) + description + "[Press any button]"; Phase 3: bars slide out + timeScale=1; uses Time.unscaledDeltaTime throughout
  - AbilityPickup.cs — floating sphere with BobAndPulse coroutine (Y-bob + Y-rotation); inner bright sphere + outer wispy shell + PointLight; OnTriggerEnter→Collect→AbilityAcquisitionPresenter.Show; post-fanfare callback grants ability flag on PlayerMovement
  - PlayerMovement.cs — CanDoubleJump was already wired at line 42 + jump state machine (lines 206, 232–241); NO changes needed
  - VerticalSliceBootstrap.cs — BuildAbilityGate/BuildHiddenPlatform/BuildAbilityPickup/BuildLorePickup/BuildStatPickup helpers; Room 1: spectral Air Dash gate at col 14 + hidden platform cols 20-22 row 10 + Double Jump orb at col 21 + INT Fragment gem at col 22 (Int+3/MaxMp+15); Room 2: cracked wall Double Jump gate at col 13 row 3 + secret ledge cols 14-17 row 3 + LorePickupTrigger; StatPickupTrigger/LorePickupTrigger inner classes
  - web-prototype/public/vertical-slice-demo.html — doubleJumpUnlocked/P_djUsed/gates[]/abilityOrbs[]/lorePickups[]/statPickups[]; double jump Z×2; gate AABB collision + dissolve; orb bob + pickup; dramatic fanfare freeze (Time_frozen) + bars + text; initRoom1Gates()/initRoom2Gates(); hidden platform + secret ledge grid tiles; golden diamond gem at col 22; proximity hint "AIR DASH →" fades in when player within 3.5 tiles of spectral gate
